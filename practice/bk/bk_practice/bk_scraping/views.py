...
def index(request) : 
	...
	for url in urls :
		...
		if url == url_wether : # 날씨정보
			...
			temps = [temp01, temp02, temp03]
			temp_class = []
			for temp in temps :
				if int(temp) < 17 :
					temp_class.append('cold')
				elif int(temp) > 27 :
					temp_class.append('hot')
				else : 
					temp_class.append('')
			...
			for idx, headline in enumerate(headlines) :
				...
				headline_news_list.append([idx + 1, headline_title, headline_link])	
		elif url == url_it_news : # IT 헤드라인 뉴스
			it_news_list = []
			it_normals = soup.find('ul', attrs={'class':'type06_headline'}).find_all('li')[:5]
			for idx, it_normal in enumerate(it_normals) :
				...
				it_news_list.append([it_normal_title, it_normal_link])
		else : # 오늘의 한줄명언(영/한)
			...
			wise_sayings = [eng_wise_sayings, eng_writer, kor_wise_sayings, kor_writer]
	...
	context = {
		...
		'headline_news' : headline_news_list,
		'it_news' : it_news_list,
		'wise_sayings' : wise_sayings
	}
	return render(request, 'bk_scraping/index.html', context)

	# template = loader.get_template('bk_scraping/index.html')
    # context = {
	# 	'sample_title' : 'web scraping.',
